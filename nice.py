...
names = ["Mark", "Pheobe", "Sally", "Michael",
         "Denise", "Ellen", "Eric", "Moana", "Lewis", "Lara"]
...
burger_prices = [7.50, 7.50, 7.50, 7.50, 8.50,
                 8.50, 8.50, 8.50, 13.50, 13.50, 13.50, 13.50]
...
def order_burgers():
    selected_burgers = []  # List to store the selected burgers' indexes
    ...
    for item in range(num_burgers):
        while num_burgers > 0:
            while True:
                try:
                    burgers_ordered = int(
                        input("Please choose your burgers by entering the number from the list: "))
                    if 1 <= burgers_ordered <= len(burger_names):
                        break
                    else:
                        print("Your number must be between 1 and",
                              len(burger_names))
                except ValueError:
                    print("That is not a valid number.")
            ...
            # Store the selected burger's index
            selected_burgers.append(burgers_ordered)
            num_burgers = num_burgers - 1

    # Print only the customer's order
    print("\nYour Order:")
    for index in selected_burgers:
        print("Ordered: {} Cost ${:.2f}".format(
            burger_names[index], burger_prices[index]))

    # Calculate and print total cost
    total_cost = sum(burger_prices[index] for index in selected_burgers)
    print("\nOrder Cost Details")
